...
@section('content')
    <div class="d-flex justify-content-between flex-wrap flex-md-nowrap align-items-center pb-2 mb-3 border-bottom">
        <h1 class="h2">{{ucfirst($group->mainEvent->name)}} -
            {{ $group->isTopLevel() ? 'všechny úkoly' : 'úkoly týmu ' . $group->name}}</h1>
        <div class="btn-toolbar mb-2 mb-md-0">
            <div class="btn-group mr-2">
                <button class="btn btn-sm btn-success"
                        data-toggle="modal" data-target="#exampleModal"><i class="fa fa-plus"></i> Nový úkol
                </button>
            </div>
        </div>
    </div>
    ...
    <div class="table-responsive">
        <table class="table table-bordered" id="data-table">
            ...
            <tbody>
            @foreach($group->mainEvent->tasks as $task)

                <tr>
                    <td><a style="color: black"
                           href="{{route('organize.task', ['group'=>$group,'task'=>$task] )}}">{{$task->name}}</a></td>
                    <td>
                    ...
                    <td>
                        @forelse($task->events->where('id','!=',$group->mainEvent->id) as $task_event)
                            <span style="color: #00a6b0; text-transform: uppercase; font-weight: bold">
                                <a href="{{route('organize.event',[$group, $task_event])}}">{{$task_event->getTypeEmoji()}} {{$task_event->name}}</a>
                            </span>
                            <br>
                        @empty
                            -
    ...
    <div class="modal fade" id="exampleModal" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel"
        ...
        <div class="modal-dialog" role="document">
            <div class="modal-content">
                ...
                <form method="POST" action="{{route('organize.tasks', $group)}}">
                    @csrf
                    <div class="modal-body">

                        <div class="form-group row">
                            <label for="name" class="col-4 col-form-label">Název úkolu</label>
                            <div class="col-8">
                                ...
                                <span id="nameHelpBlock" class="form-text text-muted">Název úkolu by měl být co nejvýstižnější.</span>
                            </div>
                        </div>
                        <div class="form-group row">
                        ...
                        <div class="form-group row">
                            <label for="select" class="col-4 col-form-label">Zodpovědné PT</label>
                            <div class="col-8">
                    ...
                    <div class="modal-footer">
                        ...
                        <button name="submit" type="submit" class="btn btn-primary">Přidat</button>
                    </div>
                </form>
            </div>
        </div>
